...
<div style="width:800px; display:block; background:#FFF; border:1px solid #000; padding:10px 5px 10px 20px; color:#000; margin:auto">
<!-- Header -->
<div class="header">
  <div class="Logo"><strong>Invoice Histor Print</strong></div>
...
//echo " Name: ".$name." Rate:".$rate." ID: ".$value."<br>";
	$runQuery = mysql_query("SELECT * FROM invoices WHERE id = '$id'")or die (mysql_error());
...
?>
     ...
     <tr class="gradeC">
      <td><?=showClientName($showD['client'])?></td>
      <td><?=$showD['ref']?></td>
      <td><?=$showD['info']?></td>
      <td><?=$showD['dates']?></td>
      <td><?=$showD['nots']?> Truck(s)</td>
      <td>₦<?=doNum($showD['amount'])?></td>
