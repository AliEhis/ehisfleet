...
<!-- Header -->
<div class="header">
  <div class="Logo"><strong>Transaction Histor Print</strong></div>
...
//echo " Name: ".$name." Rate:".$rate." ID: ".$value."<br>";
	$runQuery = mysql_query("SELECT * FROM transact WHERE id = '$id'")or die (mysql_error());
...
?>
     ...
     <tr class="gradeC">
      <td><?=showClientName($showD['client_id'])?></td>
      <td><?=$showD['pv']?></td>
      <td><?=$showD['dates']?></td>
      <td><?=showDoC($showD['debit'])?></td>
      <td><?=showAmount($showD['debit'], $showD['credit'])?></td>
