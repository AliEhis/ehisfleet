...
<!-- Header -->
<div class="header">
  <div class="Logo"><strong>Job History</strong> </div>
...
//echo " Name: ".$name." Rate:".$rate." ID: ".$value."<br>";
	$runQuery = mysql_query("SELECT * FROM fhistory WHERE regNo = '$id'")or die (mysql_error());
...
//echo $value;
	...
	while ($showD = mysql_fetch_array($runQuery))
	  {
		  $tax =  ($showD['rate'] * $showD['trips']) * ($showD['vat'] / 100);
 	      $total +=  ($showD['rate'] * $showD['trips']) + $tax;
		  
		  
	  
	  }
	  $exp = mysql_query("SELECT SUM(amount)  FROM payment WHERE ref = '$id'")or die (mysql_error());
	  
	  $expenses = mysql_fetch_array($exp);
	  ?>
      <br />
      <br />
      
	  Estimated Income: ₦<?=doNum($total)?><br />
      Estimated Expenses:₦<?=doNum($expenses['SUM(amount)'])?>
      <?php
